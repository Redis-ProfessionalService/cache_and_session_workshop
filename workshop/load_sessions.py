...
VISITED = ['www.megashop.com','www.whatever.org','redis.io','mail.example.com','duckduckgo.com','www.example.shop','redis.io','google.com','www.example.com','login.example.com','portal.sample.com']
...
class LoadSessions():
    ...
    def load_premade():
        ...
        redis.json().set('session:1', Path.root_path(), '{"lastAccessed":1723573333,"creation":1723565555,"user": {"firstname":"Jane","lastname":"Appleseed"},"visited":["redis.io","www.sample.com"], "location": "-87.6403,41.8787", "cart":[{"itemId":"invictajolly","itemCost":68.99,"quantity":1}, {"itemId":"MacBook","itemCost":2990.99,"quantity":1}]}')
        redis.json().set('session:2', Path.root_path(), '{"lastAccessed":1723572020,"creation":1723561234,"user": {"firstname":"Jonny","lastname":"Appleseed"},"visited":["redis.io","www.example.com"], "location": "-87.6229,41.8988", "cart":[{"itemId":"roncowidget","itemCost":88.88,"quantity":1}, {"itemId":"blahblah","itemCost":45.67,"quantity":4}]}' )
        redis.json().set('session:3', Path.root_path(), '{"lastAccessed":1723574040,"creation":1723572000,"user": {"firstname":"Paul","lastname":"Bunion"},"visited":["www.megashop.com","www.whatever.org","login.example.com"], "location": "-87.6359,41.8788", "cart":[{"itemId":"bigaxe","itemCost":918.99,"quantity":2}]}' )
        redis.json().set('session:4', Path.root_path(), '{"lastAccessed":1723572101,"creation":1723568404,"user": {"firstname":"Carl","lastname":"Llama"},"visited":["redis.io","portal.example.com"], "location": "-87.605,41.892"}' )
    ...
    def load_session(how_many):
        ...
        def get_create_time():
            # returns a random time between 2:00:00 pm, Aug 13, 2024 and the first presentation of this workshop (14:00:00, same day)
